...
#       valid_attrs = %{status: true, time: ~N[2023-10-23 10:14:00]}

#       assert {:ok, %Clock{} = clock} = Clocks.create_clock(valid_attrs)
#       assert clock.status == true
#       assert clock.time == ~N[2023-10-23 10:14:00]
#     end
...
#       clock = clock_fixture()
#       update_attrs = %{status: false, time: ~N[2023-10-24 10:14:00]}

#       assert {:ok, %Clock{} = clock} = Clocks.update_clock(clock, update_attrs)
#       assert clock.status == false
#       assert clock.time == ~N[2023-10-24 10:14:00]
#     end
