<template>
    <div class="bg-[#161717] min-h-screen w-16 flex flex-col items-center gap-24">
      <ul class="flex flex-col justify-around gap-12">
        <li>
          <router-link class="flex gap-6 items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group" :to="'/dashboard' + this.routes.workingtimes">
        ...
        <li>
          <router-link class="flex gap-6 items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group" :to="'/dashboard' + this.routes.chartmanager">
...
export default {
  ...
  data() {
    return {
      authenticated_user: JSON.parse(authentication_service.get_user()),
      routes: {}
    }
  },
  mounted() {
    if (this.authenticated_user.role === "employee") {
      this.routes.workingtimes = `/workingtimes/${this.authenticated_user.id}`;
      this.routes.chartmanager = `/chartmanager/${this.authenticated_user.id}`;
    } else {
      const user_id = this.$route.params.userID; 

      this.routes.workingtimes = `/workingtimes/${user_id}`;
      this.routes.chartmanager = `/chartmanager/${user_id}`;
    }
  }
};
</script>
